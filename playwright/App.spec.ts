...
test.describe('Investment Calculator App', () => {
  ...
  test('submits the form and displays results with correct calculations', async ({ page }) => {
    ...
    await page.getByLabel('Duration (Years):').fill('3');
    ...
    await expect(page.getByRole('cell', { name: 'Year 2', exact: true })).toBeVisible();
    await expect(page.getByRole('cell', { name: 'Year 3', exact: true })).toBeVisible();
  });
});